<template>
  <ElementLayout>
    <template #element>
      <VaSwitch v-bind="props" :model-value="value" @update:model-value="handleInput" v-on="listeners" ref="input" :error="isError">
...
const props = {
  ...omit(extractComponentProps(VaSwitch), propsToOmit),
}

export default defineVuesticElement({ name: 'VaSwitchElement', components: [VaSwitch], emits: VaSwitch.emits, props, propsToOmit })
</script>
