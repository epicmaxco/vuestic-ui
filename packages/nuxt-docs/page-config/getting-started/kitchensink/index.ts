...
const resolveConfigs = async () => {
  return (await Promise.all(Object
    ...
    .map(async (name) => {
      ...
      if (!exampleBlock || !titleBlock) { return }

      if (EXCLUDED_COMPONENTS.includes(titleBlock.text)) { return }

      const path = `/ui-elements/${name.replace('/page-config.ts', '').replace('./', '')}`
    ...
    }))).filter(Boolean).flat(1) as BaseBlock[]
}
