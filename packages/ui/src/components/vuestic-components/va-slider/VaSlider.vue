    ...
    >
      ...
      <template v-if="isRange">
        ...
        <div
          ...
          @focus="isFocused = true, currentSliderDotIndex = 0"
        ...
        <div
          ...
          @focus="isFocused = true, currentSliderDotIndex = 1"
          @blur="isFocused = false"
...
class SliderProps {
  range = prop({ type: Boolean, default: false })
  modelValue = prop<number | number[]>({ type: [Number, Array], default: () => [] })
  trackLabel = prop({ type: String, default: '' })
  color = prop({ type: String, default: '' })
  trackColor = prop({ type: String, default: '' })
  labelColor = prop({ type: String, default: '' })
  trackLabelVisible = prop({ type: Boolean, default: false })
  min = prop({ type: Number, default: 0 })
  max = prop({ type: Number, default: 100 })
  step = prop<number>({ type: Number, default: 1 })
  label = prop({ type: String, default: '' })
  invertLabel = prop({ type: Boolean, default: false })
  disabled = prop({ type: Boolean, default: false })
  readonly = prop({ type: Boolean, default: false })
  pins = prop({ type: Boolean, default: false })
  iconPrepend = prop({ type: String, default: '' })
  iconAppend = prop({ type: String, default: '' })
  vertical = prop({ type: Boolean, default: false })
  showTrack = prop({ type: Boolean, default: true })
}
...
export default class VaSlider extends mixins(
  ...
  setValueOnPos (pixelPosition: any) {
    ...
    ;(this.isRange ? (this.currentSliderDotIndex ? this.dot1 : this.dot0) : this.dot).focus()
